...
export default class stockPage extends React.Component {
      ...
      fetch(stockApi)
        ...
        .then(result => {
          if (typeof result["Note"] === "undefined") {
            for (
              let i = Object.keys(result["Time Series (1min)"]).length - 1; i > 0; i--
            ) {
              chartData1.push(
                parseFloat(
              ...
              labels.push(
                Object.keys(result["Time Series (1min)"])[parseInt(i)].split(" ")[1]
                .slice(0, -3)
              );
            }
          } else {
            setTimeout(() => {
              ...
              fetch(stockApi)
                ...
                .then(result => {
                  for (
                    let i =
                      Object.keys(result["Time Series (1min)"]).length - 1; i > 0; i--
                  ) {
                    chartData1.push(
                      parseFloat(
                        result["Time Series (1min)"][
  ...
  handleBuyStock(num) {
    ...
    let citiesRef = db.collection("users").doc(user).collection("stocks")
    let query = citiesRef.where('symbol', '==', symbol).get()
      .then(snapshot => {
        ...
        finalSharesTotal = parseInt(m) + parseInt(num)
        console.log("F: " + finalSharesTotal)
        ...
        db.collection("users")
          ...
          .then(() => {
            firebase
              ...
              .set({
                symbol,
                moneyPaid: (Number(finalSharesTotal) * Number(this.state.latestPrice)).toFixed(
                  2
                ),
                shares: finalSharesTotal,
                value: (Number(finalSharesTotal) * Number(this.state.latestPrice)).toFixed(2)
              })
              .catch(error => {
          ...
          .then(() => {
            ...
            db.collection("users")
              ...
              .then((snapshot) => {
                ...
                if (this.state.fundsWithoutCommas < 0) return
    ...
    return ( <
      section className = "stock" > {
        ...
        this.state.buyConfirmation === true && ( <
          ...
          div >
          <
          button className = "stockPage__buy-button"
          onClick = {
            () => {
              if (
                this.buyInput.current.value * this.state.latestPrice <=
          ...
          CONFIRM <
          /button> <
          button className = "stockPage__buy-button cancel"
          onClick = {
            () => {
              if (this._isMounted) {
                this.setState({
        ...
        this.state.valid && ( <
          ...
          Topbar / > {
            this.state.loaded ? ( <
              ...
              <
              h6 className = "stockPage__option"
              ref = {
                this.years
              }
              id = "2y"
              ...
              <
              h6 className = "active stockPage__option"
              ref = {
                this.ytd
              }
              id = "ytd"
        ...
        this.state.valid === false && ( <
          ...
          <
          svg enableBackground = "new 0 0 250.313 250.313"
          version = "1.1"
