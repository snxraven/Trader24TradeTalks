...
export default class CryptoPortfolio extends React.Component {
  _isMounted = false;
  constructor(props) {
    super(props);
    this.state = {
  ...
  getLatestPrice(symbol, i) {
    const lastPrice = `https://api.cryptonator.com/api/ticker/${symbol}`;
    fetch(lastPrice)
      .then(res => res.json())
      .then(result => {
        value[parseInt(i)] = parseFloat(
          Number(shares[parseInt(i)] * result.ticker.price).toFixed(2)
        );
      })
      .then(() => {
      ...
      firebase
        ...
        .then(
          function() {
            if (this._isMounted)
              this.setState({
                funds: Number(this.state.funds) + Number(value[parseInt(number)])
              });
            firebase
              .firestore()
              .collection("users")
  ...
  render() {
    return ( <
          ...
          this.state.loader1 === true && symbols.length > 0 && ( <
              ...
              symbols.map((val, index) => {
                return ( <
                  ...
                  <
                  td > {
                    val.toUpperCase().replace("-USD","")
                  } < /td> <
                  td > {
                    shares[parseInt(index)]
                  } < /td> <
                  td style = {
                    {
                      color: color[parseInt(index)]
                    }
                  } > {
        ...
        this.state.loader1 === "nothing" && ( <
          ...
          /g> < /
          svg > <
          h3 > You didn 't buy any crypto yet.</h3> < /
          div >
        )
      } <
      /div> < /
